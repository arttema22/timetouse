...
<body
    ...
    <!-- Hero Section -->
    <section class="bg-gradient-to-r from-blue-600 to-teal-500 text-white py-16 md:py-24">
        <div class="max-w-7xl mx-auto px-4 text-center">
            <h1 class="text-4xl md:text-6xl font-bold mb-4">
                Найдите и забронируйте<br>
                <span class="text-yellow-200">идеальный транспорт</span> для отдыха
            </h1>
            <p class="text-xl md:text-2xl mb-8 opacity-90">
                Катера, яхты, квадроциклы, снегоходы и мопеды — всё в одном месте.
            </p>

            <!-- Поисковая форма -->
            <form method="GET" class="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-4 md:p-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <!-- Место -->
                    <div>
                        <label for="location" class="block text-sm font-medium text-gray-700 mb-1">Место</label>
                        <input type="text" name="location" id="location" placeholder="Например: Сочи, Ладожское озеро"
                            class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                    </div>

                    <!-- Дата и время -->
                    <div>
                        <label for="start_time" class="block text-sm font-medium text-gray-700 mb-1">Начало</label>
            ...
            <p class="mt-6 text-sm opacity-90">
                Не зарегистрированы? <a href="{{ route('register') }}" class="underline hover:text-yellow-200">Создайте
                    аккаунт за 10 секунд</a>
            </p>
        </div>
    </section>
    ...
    <section class="py-12 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4">
            <h2 class="text-3xl font-bold text-center mb-12">Популярные места для отдыха</h2>
            <div class="grid grid-cols-2 md:grid-cols-5 gap-6">
                @foreach(['Сочи', 'Крым', 'Байкал', 'Ладожское озеро', 'Куршская коса'] as $location)
                <a href="" class="text-center p-6 bg-white rounded-xl shadow hover:shadow-lg transition">
    ...
    <section class="py-12">
        <div class="max-w-7xl mx-auto px-4">
            <h2 class="text-3xl font-bold text-center mb-12">Популярные транспортные средства</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach(\App\Models\Transport::where('is_active', true)->orderBy('created_at', 'desc')->limit(3)->get()
                as $transport)
                <div class="bg-white rounded-xl shadow overflow-hidden hover:shadow-xl transition">
                    <img src="https://placehold.co/400x250/3b82f6/ffffff?text={{ urlencode($transport->name) }}"
                        alt="{{ $transport->name }}" class="w-full h-48 object-cover">
                    <div class="p-6">
                        <h3 class="text-xl font-bold">{{ $transport->name }}</h3>
                        <p class="text-gray-600 mb-2">{{ ucfirst($transport->type) }} • {{ $transport->location }}</p>
                        <p class="text-blue-600 font-bold">
                            {{ $transport->price_per_hour ? $transport->price_per_hour . ' ₽/ч' : 'Цена по запросу' }}
                        </p>
    ...
    <section class="bg-gray-900 text-white py-16 text-center">
        <div class="max-w-4xl mx-auto">
            ...
            <p class="text-xl mb-8 opacity-90">Зарегистрируйтесь как клиент — и найдите идеальный транспорт за минуту.
            </p>
            <a href="{{ route('register') }}"
                class="bg-green-600 hover:bg-green-700 text-white font-bold py-4 px-8 rounded-lg text-lg transition">
